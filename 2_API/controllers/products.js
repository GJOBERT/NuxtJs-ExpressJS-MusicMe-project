...
exports.searchProduct = (req, res, next) => {
	...
	if (search.order === 'ASC') {
		...
		const inserts = [search.main, search.main, search.main, search.main, search.price, search.category_id, search.col];
		...
		if (err) {
			...
			if (result.length > 0) {
				...
				if (reqBody.image !== null && reqBody.image !== "" && reqBody.image !== undefined) {
					...
					const inserts = [reqBody.title, reqBody.description, reqBody.artist, reqBody.year, reqBody.price, reqBody.num_available, reqBody.num_reserved, reqBody.num_total, reqBody.category_id, id]
